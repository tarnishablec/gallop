`gallop`并没有像`React`那样使用[合成事件](https://react.docschina.org/docs/events.html)。在[模板](/#Template)中绑定的所有事件都是原生事件或者是[自定义事件](https://developer.mozilla.org/en-US/docs/Web/API/CustomEvent)。
